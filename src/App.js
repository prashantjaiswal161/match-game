...
class App extends Component {
  ...
  render() {
    const {activeTabId, score, timer, randomNumber} = this.state
    const filteredImages = this.getFilteredImages()
    if (timer === 0) {
      this.componentWillUnmount()
    }
    console.log(randomNumber)
    const randomImageId = imagesList[randomNumber].id
    return (
      <div>
        ...
        <div className="bg-container">
          {timer !== 0 ? (
            <div>
              <img
                key={imagesList[randomNumber].imageUrl}
